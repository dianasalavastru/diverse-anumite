...
113. **LAUNCH EXCEPTION — public production dataset accepted for PUBLIC SERVICE CONTENT ONLY.** Owner decision, 2026-09-25. The project's Sanity plan does not allow private datasets (`ACL mode "private" not allowed`), and the plan is not upgraded for launch. **This departs from §18.1 for one content type only:** the `production` dataset may hold the six Service documents, whose every field is copy intended for public publication. **Work Entries, client records, publication-clearance data (`capturePublicationCleared`), contact or personal data, and any other potentially non-public content remain PROHIBITED in the public dataset** until a privacy-capable content strategy exists (a plan with private datasets, or an equivalent split). The six Service documents were created as drafts (`drafts.service-<key>`, `slug.ro` = key, `enPublished: false`) from the locked Stable RO content, validated with zero errors, and await owner publication in Studio. No Work document is created: there are no publication-cleared assets, so the initial launch has zero Projects, and portfolio population is post-launch.

114. **Contact actions exist only when Contact is actionable.** While neither the enquiry form nor a confirmed direct channel exists (`isContactActionable`), every CTA whose sole destination is `/contact` is omitted — Service S-5, hub H-6, Homepage M-6, Work Entry onward and the About closing — without changing their locked strings; they return automatically when a form or a channel exists. Where omission would leave a station with only its marker (hub H-6), the station is omitted rather than rendered empty; locked rail numbers are not changed. Header/footer `Contact` navigation stays.
